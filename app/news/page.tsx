                    ...
                    <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
                        ...
                        <div className="flex flex-wrap gap-2">
                            <button
                                ...
                                className={`px-4 py-2 rounded-full text-sm font-medium transition-colors font-primary ${!currentTag
                            ...
                            {tags.map((tag) => (
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-full text-sm font-medium transition-colors font-primary ${currentTag === tag
                                        ? 'bg-primary text-white'
                                        : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                                        }`}
                                    style={{ borderRadius: 'var(--border-radius)' }}
            ...
            <div className="container-limited py-12">
                ...
                {news.length === 0 ? (
                    <div className="text-center py-12">
                        ...
                        <h3 className="mt-2 text-sm font-medium text-gray-900 font-heading">Không có tin tức</h3>
                        <p className="mt-1 text-sm text-gray-500 font-primary">
                            {currentSearch || currentTag ? 'Không tìm thấy tin tức phù hợp với bộ lọc.' : 'Chưa có tin tức nào được đăng.'}
                        </p>
                    </div>
                ) : (
                        ...
                        <nav className="flex space-x-2">
                            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-lg font-medium transition-colors font-primary ${page === currentPage
                                        ? 'bg-primary text-white'
...
const NewsCard = ({ news }: { news: News }) => {
    ...
    const excerpt = news.excerpt || newsService.truncateContent(newsService.stripHtml(news.content));
            ...
            <article className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow overflow-hidden" style={{ borderRadius: 'var(--border-radius)' }}>
                ...
                <div className="p-6">
                    <div className="flex items-center text-sm text-gray-500 mb-3 font-primary">
                        <time>{newsService.formatDate(news.publishedAt || news.createdAt)}</time>
