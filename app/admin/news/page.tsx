...
import { CreateNewsRequest, News, NewsQueryParams, UpdateNewsRequest } from '@/types';
...
const NewsManagement = () => {
        ...
        try {
            ...
            if (editingNews) {
                ...
                if (selectedImage) {
                    ...
                    response = await newsService.updateNewsWithImage(editingNews.id, formData as UpdateNewsRequest, selectedImage);
                } else {
                    // Update without changing image
                    response = await newsService.updateNews(editingNews.id, formData as UpdateNewsRequest);
                }
                ...
                if (selectedImage) {
                    // Create with image
                    response = await newsService.createNewsWithImage(formData, selectedImage);
                } else {
    ...
    const handleEdit = (newsItem: News) => {
        ...
        setFormData({
            ...
            publishedAt: newsItem.publishedAt ? new Date(newsItem.publishedAt).toISOString().slice(0, 16) : ''
        });
        setShowModal(true);
    };
            ...
            <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
                {loading ? (
                    ...
                    <div className="text-center py-12">
                        ...
                        <p className="mt-1 text-sm text-gray-500">Bắt đầu bằng cách tạo tin tức đầu tiên.</p>
                    </div>
                ) : (
                                ...
                                {news.map((item) => (
                                    <tr key={item.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${item.isPublished
                        ...
                        <div className="p-6">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                        ...
                                        {imagePreview ? (
                                            <div className="space-y-4">
                                                ...
                                                <p className="text-xs text-gray-500">
                                                    {editingNews ? 'Ảnh này sẽ thay thế ảnh hiện tại' : 'Ảnh này sẽ được sử dụng làm ảnh đại diện'}
                                                </p>
                                                <button
                                                    type="button"
                                                    onClick={(e) => {
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 mt-1">
                                        Sử dụng Markdown để định dạng nội dung. Hỗ trợ **bold**, *italic*, # headers, links, images, v.v.
                                    </p>
                                </div>
                                ...
                                <div className="flex justify-end space-x-4 pt-6 border-t">
                                    ...
                                    >
                                        {loading ? 'Đang lưu...' : (editingNews ? 'Cập nhật' : 'Tạo mới')}
                                    </button>
                                </div>
                            </form>
                        </div>
