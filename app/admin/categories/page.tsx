    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        try {
            if (selectedImage) {
                ...
                const response = await categoryApi.updateWithImage(editingCategory.id, formDataToSend);
                if (response.success) {
                    await fetchCategories();
                ...
                const response = await categoryService.updateCategory(editingCategory.id, updateData);
                if (response.success) {
                    await fetchCategories();
                    setEditingCategory(null);
                    setFormData({ name: '', description: '', imageUrl: '', parentId: undefined });
            ...
            <div className="bg-white rounded-lg shadow overflow-hidden">
                ...
                {categories.length === 0 && (
                    <div className="text-center py-12">
                        ...
                        <p className="mt-1 text-sm text-gray-500">Bắt đầu bằng cách tạo danh mục đầu tiên.</p>
                    </div>
                )}
            </div>
            ...
            {(showCreateModal || editingCategory) && (
                <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
                    ...
                    `}
                                    ...
                                    {(imagePreviewUrl || (editingCategory?.imageUrl && !selectedImage)) && (
                                        <div className="mt-4">
                                            <div className="relative inline-block">
                                                <Image
                                                    src={imagePreviewUrl || (editingCategory?.imageUrl || '/image/noimage.png')}
                                            ...
                                            {imagePreviewUrl && (
                                                <p className="text-xs text-green-600 mt-1">Hình ảnh mới đã chọn</p>
                                            )}
                                        </div>
                                    )}
                                </div>
