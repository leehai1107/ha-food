...
const ProductManagement: React.FC = () => {
  ...
  const handleCreateProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formDataToSend.append(
        "slug",
        generateSlug(formData.productName, formData.productSku)
      );
      formDataToSend.append(
        "quantity",
        (Number(formData.quantity) || 0).toString()
      );
      formDataToSend.append("productType", formData.productType);
      formDataToSend.append(
        "originalPrice",
        (formData.originalPrice || 0).toString()
      );
      formDataToSend.append(
        "currentPrice",
        (formData.currentPrice || 0).toString()
      );
      formDataToSend.append(
        "productDescriptions",
        formData.productDescriptions || ""
      );
      ...
      formDataToSend.append(
        "available",
        (formData.available ?? true).toString()
      );
      ...
      const ingredients = (formData.productIngredients || []).filter(
      ...
      const response =
        selectedImages.length > 0
          ...
          : await productService.createProduct({
              ...
              productIngredients: (formData.productIngredients || []).filter(
                (ingredient) => ingredient.trim() !== ""
              ),
  ...
  const handleUpdateProduct = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedImages.length > 0) {
        ...
        const response = await productApi.updateWithImages(
          editingProduct.productSku,
          formDataToSend
        );
        if (response.success) {
          await fetchData();
  ...
  const openEditModal = async (product: Product) => {
    ...
    setFormData({
      ...
      slug:
        product.slug || generateSlug(product.productName, product.productSku),
  ...
  const addIngredient = () => {
    if (
      ...
      !(formData.productIngredients || []).includes(ingredientInput.trim())
    ) {
      setFormData((prev) => ({
        ...prev,
        productIngredients: [
          ...(prev.productIngredients || []),
          ingredientInput.trim(),
        ],
      }));
      setIngredientInput("");
  ...
  const handleAddReview = async () => {
    ...
    try {
      const response = await productService.createReview(
        editingProduct.productSku,
        {
          ...reviewForm,
